...
<%-- 			${page.paging.pageNo} --%>
			<c:set var="la" value="${page.paging.lastPage}"/>
		...
		<nav class="navbar nav3">
			...
			<c:choose>
				<c:when test="${!empty no}">
					<li><a href="#" id="newnote3" class="btn btn-default btn-sm">수정  <span class="glyphicon glyphicon-plus "></span></a></li>
					<li><a href="#" id="newnote4" class="btn btn-default btn-sm">삭제 <span class="glyphicon glyphicon-plus "></span></a></li>
				</c:when>
				
				<c:otherwise>
				
					<li><a href="#" id="newnote2" class="btn btn-default btn-sm">저장 <span class="glyphicon glyphicon-plus "></span></a></li>
				</c:otherwise>
			</c:choose>
				</ul>
			...
			<div class="col-lg-10 list1">
				<c:choose>
					...
					<button class="btn btn-warning btn-xs"
						onclick="location.href='/note/modify/${city.id}?pageNo=${param.pageNo}'">수정</button>
  		
  		
  					<form action="note/unregister/${note.noteNo}?pageNo=${param.pageNo}" method="post">
					<button  class="btn btn-danger btn-xs" type="submit" value="삭제">삭제</button>
					</form>
  					
<!--   					<button class="btn btn-danger btn-xs" -->
<%--   						onclick="location.href='/note/unregister/${note.noteNo}?pageNo=${param.pageNo}'">삭제</button> --%>
					</div>
					</c:when>
					
					<c:otherwise>
<%-- 							<jsp:useBean id="now" class="java.util.Date"/> --%>
<%-- 							<form:input path="noteDate"  value="${now}" /> --%>
<%-- 									<input type="text"  value="${now}" > --%>
						<form:form action="/note/reg?pageNo=${page.paging.pageNo}" method="post" modelAttribute="noteForm">
							<div class="input-group">
								<span class="input-group-addon">title:</span>
			...
			<c:if test="${page.paging.lastPage != la-1}">
